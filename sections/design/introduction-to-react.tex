...
An example of this can be seen in the following listings:
...
\\\\
\subsection*{Props}
When a component is instantiated it is possible to pass an arbitrary number of arguments to the component by passing key and value pairs. These are then accessed through the props object in the component. An example of this can be seen in the following listing.

\begin{lstlisting}[label={lst:react-props-lsting}]
function Site(){
...
Functional components can also be stateful, but the choice of using class components is mostly a question about preferences.
...
An example of how this is done can be seen in the following listing.
\begin{lstlisting}
...
This is useful for when we need to do fetch data from the API that we want to save in the state of the component.
The reason that this is not done in the constructor of the component is because, the render function should not be waiting for asynchronous operations before it is allowed to render.
This is because we do not know how long it will take before function is done executing or if it will even succeed.
So the rendering of the component should not be waiting for this.
Instead it is better to first render the component before calling asynchronous functions that will trigger a re-rendering of the component.
\\\\
