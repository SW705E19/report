...
First one is a smart component, also named containers.
...
The second a presentational component.
These are most often stateless and called presentational due to all they do is output UI elements \cite{Vumbula-react}.
\\\\
The container for the create user can be seen on \autoref{create-user}.
...
The UserForm can be seen on \autoref{user-form}.
This presentational component does not have any states, but it uses the props that were passed by where the user form was declared.
Within the \texttt{TextField} it checks through props if the first name is valid. 
If it is not valid, and error will be should, and the \texttt{helperText} will also be shown.
Whenever the \texttt{TextField} is changed the handleChange is called, and as it changes the state, the page will be rendered again.
