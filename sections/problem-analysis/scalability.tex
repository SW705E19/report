...
\begin{itemize}
    ...
    This way of scalability will decrease synchronism in the application but allows the system to be available to all users, even during a network partition.
The results in a situation where the users may not read the most recently updated data, but it will still be available to them during a network failure. As the network partition becomes resolved, consistency will be restored.
    ...
    Different kinds of platforms and tools can be used to facilitate this by monitoring the application and ensuring that the system is operating correctly. 
...
\begin{itemize}
    ...
    The API should work as a service so that it can be used for any application such as a smartphone application or a web site. 
    ...
    \item As mentioned in scalability in availability, the application does not need to provide the newest data available at all times if the users do not need to know. 
